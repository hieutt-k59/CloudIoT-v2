...
def get_tem(request, device_name):
    loc = 0
    arr = {"data":[]}
    a = []
    if request.method == "GET":
        loc = request.GET.get("loc")
    document_list = db.devices.find({'device':device_name, 'type':'tem'})
    for document in document_list:
        t = datetime.strptime(document['timestamp'], '%Y-%m-%d %H:%M:%S.%f')
        a = [int(t.year), int(t.month), int(t.day), int(t.hour), int(t.minute), float(document.temp)]
        arr["data"].append(a)
    
    return HttpResponse(json.dumps(arr["data"][int(loc):]), content_type='application/json')

def get_humi(request, device_name):
    loc = 0
    sensorID = ""
    arr = {"data":[]}
    a = []
    if request.method == "GET":
        loc = request.GET.get("loc")
    document_list = db.devices.find({'device':device_name, 'type':'humi'})
    for document in document_list:
        t = datetime.strptime(document['timestamp'], '%Y-%m-%d %H:%M:%S.%f')
        a = [int(t.year), int(t.month), int(t.day), int(t.hour), int(t.minute), float(document.temp)]
        arr["data"].append(a)
...
def show_chart(request, device_name):
    content = {'sensorID': device_name}
...
def ListSensor(request, gatewayID):
    try:
        ...
        pageSensor = request.GET.geavatar.pngt('page',1)
        try:
...
def devices(request, facilityID):
    ...
    try:
        ...
        if '_edit' in request.POST:
            for a in data:
                if request.POST.get('%d' % a.deviceid):
                    return redirect('equipmentEdit', deviceID=a.deviceid)
        if '_delete' in request.POST:
            for a in data:
                if request.POST.get('%d' % a.deviceidid):
                    a.delete()
            return redirect('', facilityID=facilityID)
        if '_new' in request.POST:
            return redirect('newdevices', facilityID=facilityID)
    except Exception as e:
        print(e)
        raise Http404
    return render(request, 'FacilityUI/Devices/device_home.html',
                  {'page': 'listDevices', 'obj': obj, 'facilityID': facilityID, 'siteID': faci.siteid_id, 'faci': faci,
